...
export const OperatorMetricsDashboard: React.FC<OperatorMetricsDashboardProps> = React.memo(({ history }) => {
  const series = useMemo(
    () =>
      METRICS.map((metric) => {
        const values = history.map((sample) => sample[metric.key]);
  ...
  return (
    <div className="h-full overflow-hidden border border-current/20 bg-black/60 p-3 font-mono text-xs">
      ...
      <div className="grid h-[calc(100%-2rem)] grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2">
