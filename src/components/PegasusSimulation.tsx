...
export const PegasusSimulation: React.FC<PegasusSimulationProps> = ({ 
  ...
  // Initialize real audio analysis
  useEffect(() => {
      ...
      try {
        ...
        const updateAudioLevel = () => {
          if (analyserRef.current) {
            ...
            // Detect breathing patterns from low-frequency audio
            const lowFreqSum = dataArray.slice(0, 10).reduce((sum, value) => sum + value, 0);
            setBreathPattern(Math.sin(Date.now() / 3000) * 0.3 + (lowFreqSum / 2550));
            
            // Estimate pulse from audio variations
            const midFreqSum = dataArray.slice(10, 30).reduce((sum, value) => sum + value, 0);
            setPulseRate(72 + Math.sin(Date.now() / 1000) * 8 + (midFreqSum / 255) * 10);
          }
